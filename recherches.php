...
<div>
    ...
    <table class="table table-striped table-hover" id="grande_boite" >
        ...
        if (isset($_GET['categorie'])&& isset($_GET['critere'])) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM dossiers_patients WHERE $categorie LIKE ?");
			...
			<td >
			<?php echo $value['photo_analyse'] ?></td></tr>
			<tr>
			...
			<td>
                       <button class="filled-btn"> <a href="modifier_dossier_patient.php?id=<?php echo $value['id']?>" class="btn">modifier</a></button>
                        <button class="sup-btn"><a href="supprimer_dossier_patient.php?id=<?php echo $value['id']?>" onclick="return confirm('Voulez-vous supprimer ?');" class="btn">
						supprimer</a></button>
                    </td>
                </tr>
                <?php
